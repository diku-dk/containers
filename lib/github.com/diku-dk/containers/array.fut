...
local
def create_sample [n] 't
                  (arr: [n]t)
                  (rng: rng)
                  (sample_size: i64) : (rng, [sample_size]t) =
  ...
  let (reduction, _, _) =
    ...
    while length not_reduced != 0 do
      ...
      let (new_reduced, new_not_reduced) =
        ...
        partition (\(h', elem) -> not_reduced[collision_idxs[h']].0 `eq` elem.0)
  ...
  let (uniques, _, _) =
    ...
    while length elems != 0 do
      ...
      let new_elems =
        elems
        |> zip hashes
        |> filter (\(h', v) -> not (elems[collision_idxs[h']] `eq` v))
        |> map (.1)
...
-- array it can instead correspond to a key. Here an array of `n`@term
...
-- and commutative operator `op`@term, futhermore an neutral element
-- `ne`@term must be given. To perform this reduction a definition of
-- key equality must be given `eq`@term, this is done as to figure out
-- which elements will be reduced. And a good hash function
-- `hash`@term must be chosen, see `hash_i64`@term an for
-- example. Given a hash function with an uniform distribution then
-- this function has the expected work of *O(n * W(op))* where *W(op)*
-- is the work of `op`.  The span of this operation is unknown since
...
-- faster. Therefore, it also needs a hash function `hash` with a
-- uniform distribution. Its work is *O(n)* and its span is unknown
...
-- compiled random input { [100000000]i32 }
local
entry bench_dedup_mod [n] (arr: [n]i32) =
...
-- script input { replicate_i32 100000000i64 1i32 }
local
entry bench_count_occourences_i32 [n] (arr: [n]i32) =
...
-- compiled random input { [100000000]i32 }
local
entry bench_count_occourences_mod_i32 [n] (arr: [n]i32) =
...
-- compiled random input { [100000000]i32 }
local
entry bench_count_occourences_radix_sort_and_segscan_i32 [n] (arr: [n]i32) =
...
local
entry bench_count_occourences_i64 [n] (arr: [n]i64) =
...
-- compiled random input { [100000000]i64 }
local
entry bench_count_occourences_mod_i64 [n] (arr: [n]i64) =
...
-- compiled random input { [100000000]i64 }
local
entry bench_count_occourences_radix_sort_and_segscan_i64 [n] (arr: [n]i64) =
