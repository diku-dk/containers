...
--
-- Count-Min Sketch [1] is a probabilistic data structure for estimating the
-- frequency of elements in a data stream. It uses `d` hash functions each
-- mapping elements to one of `w` buckets, giving a matrix of counters of shape
-- `[d][w]`. Frequency queries return the minimum counter across all rows, which
-- gives an upper bound on the true frequency.
--
-- The error in frequency estimates is within ε = e/w with probability 1 - δ
-- where δ = (1/2)^d. Typical choices are w = ⌈e/ε⌉ and d = ⌈log(1/δ)⌉.
--
-- [1] Graham Cormode, S. Muthukrishnan, An improved data stream summary: the
-- count-min sketch and its applications, Journal of Algorithms, Volume 55,
-- Issue 1, 2005, Pages 58-75, ISSN 0196-6774,
...
module type count_min_sketch = {
  ...
  -- | Context to hash keys, see the hash key library for further elaboration.
  type~ ctx

  -- | The Count-Min Sketch structure with `d` hash functions and `w` buckets.
  type~ sketch [d] [w]

  -- | Create a Count-Min Sketch.
  --
  -- `d` controls the probability of error: error probability ≤ (1/2)^d.
  -- `w` controls the magnitude of error: error ≤ e/w * total_count.
  ...
  --
  -- Returns an upper bound on the true frequency. The estimate is never
  -- less than the true frequency.
  --
  -- **Work:** *O(d)*
  --
  -- **Span:** *O(1)*
